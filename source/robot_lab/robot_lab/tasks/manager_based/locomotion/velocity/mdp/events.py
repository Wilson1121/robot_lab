...
def push_by_setting_velocity(
    ...
    delta_w = math_utils.sample_uniform(ranges[:, 0], ranges[:, 1], vel_w.shape, device=asset.device)
    vel_w = vel_w + delta_w
    # set the velocities into the physics simulation
    asset.write_root_velocity_to_sim(vel_w, env_ids=env_ids)

    # Cache the sampled delta so observations can read it later in the same step.
    num_envs = env.scene.num_envs
    cache = getattr(env, "_last_push_delta_w", None)
    if cache is None or cache.shape != (num_envs, 6) or cache.device != asset.device:
        cache = torch.zeros((num_envs, 6), device=asset.device, dtype=vel_w.dtype)
        setattr(env, "_last_push_delta_w", cache)
    else:
        cache.zero_()
    cache[env_ids] = delta_w
    setattr(env, "_last_push_step", env._sim_step_counter)
